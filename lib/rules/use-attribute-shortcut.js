    ...
    create: function(context) {
        ...
        return utils.defineTemplateBodyVisitor(context, {
            'VAttribute'(node) {
                ...
                if (node.value === null || node.value.type !== 'VExpressionContainer') {
                    return;
                }
                ...
                context.report({
                    node,
                    message: getErrorMessage(node.key.argument.name),
                    fix(fixer) {
                        return fixer.replaceText(node, node.key.argument.name);
                    }
                })
            },
